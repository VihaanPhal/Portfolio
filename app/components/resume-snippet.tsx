...
export default function ResumeSnapshot() {
  ...
  // Right-panel shader mode (1 = Dots, 2 = Grain). Random start.
  type Mode = 1 | 2;
  const [mode, setMode] = useState<Mode>((Math.random() < 0.5 ? 1 : 2) as Mode);

  // Hard-coded, theme-aware shader colors
  const shaderBack = isDark ? "#000000" : "#ffffff";
  const dotsAccent = isDark ? "#f472d0" : "#4f8cff";
  const grainPalette = isDark
    ? ["#7a00ff", "#f07ad6", "#6ee7ff", "#3a0dff"]
    : ["#b18cff", "#f7b7ff", "#9ad9ff", "#86a7ff"];
          ...
          >
            ...
            <div className="mb-6">
              ...
              <div className="space-y-0.5">
                <h2 className="text-[18px] leading-5">VIHAAN DEEPAK PHAL</h2>
